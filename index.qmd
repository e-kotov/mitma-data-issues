...
listing:
  ...
  sort-ui: [date, date-modified, issue-importance, status]
  date-format: iso
  # filter-ui: [date, date-modified, title, categories, issue-number]
  # fields: [date, date-modified, title, categories, issue-number]
  filter-ui: [date, date-modified, title, categories, issue-importance, status]
  fields: [date, date-modified, title, categories, issue-importance, status]
page-layout: full
...
The data in this repository contains *"datos abiertos MITMA" ("MITMA open data")* [@mitma-mobility-2024-v6] downloaded from [this page](https://www.transportes.gob.es/ministerio/proyectos-singulares/estudios-de-movilidad-con-big-data/opendata-movilidad).
